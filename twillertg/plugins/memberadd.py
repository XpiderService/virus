...
def get_proxy_info(url, session_name, channel_username, is_private):
    ...
    for proxy in data:
        ...
        if test_proxy_connection(host, port):
            ...
            join_channel_with_session(
                proxy_config, session_name, channel_username, is_private)
...
def join_channel_with_session(proxy_config, session_name, channel_username, is_private, invite_link=None):
...
def main():
    ...
    proxy_config = get_proxy_info(
        api_url, session_names[0], channel_username, is_private)  # assuming there is at least one session name
